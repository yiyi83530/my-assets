...
function normalizeTransaction(raw) {
  ...
  const rawDate = String(raw?.date || new Date().toISOString());
  return {
    id: String(raw?.id || `tx_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
    // Sheets 的日期儲存格會被 Apps Script JSON.stringify 成完整 ISO 時間；
    // 統一保留日期部分，避免持股歷史計算再拼接時間後形成 Invalid Date。
...
function normalizeStockHoldingSnapshot(raw) {
  const monthKey = normalizeMonthKey(raw?.monthKey);
  const market = String(raw?.market || 'TWSE').toUpperCase() === 'US' ? 'US' : 'TWSE';
  const stock = String(raw?.stock || '');
  return {
    id: String(raw?.id || `holding_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
    monthKey,
    market,
    symbol: normalizeStockSymbol(raw?.symbol, stock, market),
    stock,
...
function normalizeIndustryCategory(raw) {
  const rawSymbols = Array.isArray(raw?.symbols)
    ? raw.symbols
    : String(raw?.symbols || '').split(/[\s,，、;；]+/);
  return {
    name: String(raw?.name ?? raw?.category ?? '').trim(),
    symbols: [...new Set(rawSymbols.map((symbol) => String(symbol || '').trim().toUpperCase()).filter(Boolean))],
  };
}
...
export async function fetchSheetsData(apiUrl) {
  ...
  const costBasisAdjustments = Array.isArray(data.costBasisAdjustments)
    ...
    : [];
  const stockHoldingSnapshots = Array.isArray(data.stockHoldingSnapshots)
    ? data.stockHoldingSnapshots.map(normalizeStockHoldingSnapshot)
    : [];
  const industryCategories = Array.isArray(data.industryCategories)
    ? data.industryCategories.map(normalizeIndustryCategory).filter((category) => category.name)
    : null;
  ...
  return {
    ...
    stockMarketPrices: data.stockMarketPrices && typeof data.stockMarketPrices === 'object'
      ...
      : {},
    stockMonthlyClosePrices: data.stockMonthlyClosePrices && typeof data.stockMonthlyClosePrices === 'object'
      ? data.stockMonthlyClosePrices
      : {},
    lastMonthNetWorth: toNumber(data.lastMonthNetWorth, 0),
  };
}

export async function saveIndustryCategoriesToSheets(apiUrl, categories) {
  const normalized = (Array.isArray(categories) ? categories : [])
    .map(normalizeIndustryCategory)
    .filter((category) => category.name);
  await callSheetsApi(apiUrl, 'saveIndustryCategories', { categories: normalized });
  return normalized;
}

export async function saveStockHoldingSnapshotsToSheets(apiUrl, monthKey, snapshots) {
  ...
  const normalized = (Array.isArray(snapshots) ? snapshots : [])
    .map((snapshot) => normalizeStockHoldingSnapshot({ ...snapshot, monthKey: key }))
    .filter((snapshot) => snapshot.stock && snapshot.holdingQty >= 0);

  await callSheetsApi(apiUrl, 'upsertStockHoldingSnapshots', { monthKey: key, snapshots: normalized });
  return normalized;
}
