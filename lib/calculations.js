...
function getEventTimestamp(date, recordedAt) {
  return new Date(String(recordedAt || '').startsWith(String(date || '')) ? recordedAt : `${date}T12:00:00`).getTime();
}

function inferCutoffMonth(options = {}) {
  if (options.cutoffMonth) return String(options.cutoffMonth).slice(0, 7);
  if (options.cutoffDate) return String(options.cutoffDate).slice(0, 7);
  return null;
}

function inferCutoffTimestamp(options = {}) {
  if (!options.cutoffDate) return Infinity;
  return new Date(`${String(options.cutoffDate).slice(0, 10)}T23:59:59`).getTime();
}

function getLatestSnapshotsByStock(snapshots = [], cutoffMonth = null) {
  return (snapshots || []).reduce((map, snapshot) => {
    ...
    if (!stock || !monthKey || (cutoffMonth && monthKey > cutoffMonth)) return map;

    const current = map[stock];
    if (!current || monthKey > String(current.monthKey || '')) {
      map[stock] = snapshot;
    }
    return map;
  }, {});
...
export function calculateStockPortfolio(
  ...
  const cutoffMonth = inferCutoffMonth(options);
  const cutoffTimestamp = inferCutoffTimestamp(options);
  const latestSnapshotsByStock = getLatestSnapshotsByStock(holdingSnapshots, cutoffMonth);
  ...
  const events = [
    ...(transactions || [])
      .filter((tx) => {
        ...
        const snapshotEndDate = getMonthEndDateString(snapshot.monthKey);
        return String(tx.date || '') > snapshotEndDate;
      })
      .map((tx) => ({
        kind: 'transaction',
    ...
    ...(costBasisAdjustments || [])
      .filter((adjustment) => {
        ...
        if (!snapshot) return true;
        const snapshotEndDate = getMonthEndDateString(snapshot.monthKey);
        return String(adjustment.effectiveAt || '').slice(0, 10) > snapshotEndDate;
      })
      .map((adjustment) => ({
