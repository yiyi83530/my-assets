...
export async function GET(request) {
  ...
  if (isUs) {
    ...
    // 依最常見的上市、上櫃、興櫃順序逐級查找；找到報價即停止，
    // 避免每檔股票無條件同時打三個官方來源。
    quote = await fetchRealtimeTwQuote(symbol, 'tse');
    if (!quote) quote = await fetchRealtimeTwQuote(symbol, 'otc');
    if (!quote) quote = await fetchEsbQuote(symbol);

    // 只有即時來源皆無資料時才依序使用官方日收盤資料。
    if (!quote) quote = await fetchDailyClose(symbol, TWSE_DAILY_URL, 'TWSE');
    if (!quote) quote = await fetchDailyClose(symbol, TPEX_DAILY_URL, 'TPEx');
  }
