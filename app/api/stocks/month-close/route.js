...
async function fetchYahooMonthClose(yahooSymbol, monthKey) {
  const range = getMonthRangeSeconds(monthKey);
...
export async function GET(request) {
  ...
  if (!symbol || !getMonthRangeSeconds(month)) {
    return Response.json({ price: null, status: 'unavailable', source: null, asOf: null });
  }

  const candidates = getYahooSymbols(symbol, isUs);
  for (const yahooSymbol of candidates) {
    const quote = await fetchYahooMonthClose(yahooSymbol, month);
    if (quote) {
      return Response.json({ ...quote, yahooSymbol });
    }
  }
